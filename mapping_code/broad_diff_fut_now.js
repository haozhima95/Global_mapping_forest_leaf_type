var rcplist = ['126','370','585']; // RCPs.


var modellist = ['gfdl-esm4','ipsl-cm6a-lr', 'mpi-esm1-2-hr', 'mri-esm2-0', 'ukesm1-0-ll']; // Models.
...
var alldf = rcplist.map(function(rcp){
  var dff = modellist.map(function(modelname){
    ...
    var diffeve = im.select([0]).add(im.select([1])).subtract(fft.select([0])).subtract(fft.select([1])).rename('broaddiff')
...
print(alldf.limit(10));
  Export.table.toCloudStorage({
    collection:alldf,
    description:'broad_difference_allmodel_allrcp_20221010',
    fileFormat:'CSV',
    bucket:'haozhi_ma'
  });
