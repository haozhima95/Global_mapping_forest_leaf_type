...
```{R}
dsr <- read.csv('C:\\Users\\haozh\\Desktop\\LL\\functional_type_grid_search\\cv_validation\\classifiedvalidation_fold_1_ntree250_mtry20_minleaf20_dbh_full_var_recent_occurrence_20221010.csv')
...
```{R}
dfbemean <- mean(dfwithpred$be_prop_dbh)
dfbdmean <- mean(dfwithpred$bd_prop_dbh)
dfnemean <- mean(dfwithpred$ne_prop_dbh)
dfndmean <- mean(dfwithpred$nd_prop_dbh)
...
```{R}
bc_mean <- function(ds){
  return(sqrt(ds$be_prop_dbh*ds$be_mean)+sqrt(ds$bd_prop_dbh*ds$bd_mean)+sqrt(ds$ne_prop_dbh*ds$ne_mean)+sqrt(ds$nd_prop_dbh*ds$nd_mean))
}

bc_model <- function(ds){
  return(sqrt(ds$be_prop_dbh*ds$be_pred)+sqrt(ds$bd_prop_dbh*ds$bd_pred)+sqrt(ds$ne_prop_dbh*ds$ne_pred)+sqrt(ds$nd_prop_dbh*ds$nd_pred))
}
