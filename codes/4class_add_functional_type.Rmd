...
```{R}
library(readr)
for(i in c(10,100,1000,250,500,1500,5000,9000,1200,2500)){

  
  set.seed(i)
sampleddf <- newds %>% group_by(LAT,LON, year) %>%
  sample_n(1,weight = dbh*dbh)
...
write_csv(sampleddf, paste0('C:\\Users\\haozh\\Desktop\\LL\\GFBI_data\\random_sampled_occurrence_dbh_seed',i,'_recent_withfold_with_index_20220809','.csv'))
...
ggplot()+
  ...
  geom_point(data = sampleddf, aes(x = LON, y = LAT))
...
write_csv(sampleddf, 'C:\\Users\\haozh\\Desktop\\LL\\GFBI_data\\random_sampled_occurrence_dbh_seed1000.csv')
